...
func (e Envelope) State() (EventState, error) {
	if !canonicalEventPattern.MatchString(e.Event) {
		return "", fmt.Errorf("invalid canonical event %q", e.Event)
	}
	parts := strings.Split(e.Event, ".")
	state := EventState(parts[len(parts)-1])
	switch state {
	case StateRequest, StateOK, StateError:
		return state, nil
	default:
		return "", fmt.Errorf("invalid event state %q", state)
	}
}

func (e Envelope) Validate() error {
	if e.ID == "" || e.IdempotencyKey == "" || e.SchemaID == "" || e.CorrelationID == "" {
		return errors.New("id, idempotency_key, schema_id and correlation_id are required")
	}
	if _, err := e.State(); err != nil {
		return err
	}
	switch e.Guarantee {
	case GuaranteeBestEffort, GuaranteeReceived, GuaranteeProcessed:
	default:
		return fmt.Errorf("invalid guarantee %q", e.Guarantee)
	}
...
func (e Envelope) DigestHex() (string, error) {
	d, err := e.Digest()
	if err != nil {
		return "", err
	}
...
const (
	ControlReceived     ControlKind = "RECEIVED"
	ControlSettledOK    ControlKind = "SETTLED_OK"
	ControlSettledError ControlKind = "SETTLED_ERROR"
	ControlLeaseExpired ControlKind = "LEASE_EXPIRED"
)
...
func (f ControlFrame) Validate() error {
	switch f.Kind {
	case ControlReceived, ControlSettledOK, ControlSettledError, ControlLeaseExpired:
	default:
		return fmt.Errorf("invalid control kind %q", f.Kind)
	}
	if f.EventID == "" || f.WorkerID == "" || f.FencingToken == 0 {
		return errors.New("event_id, worker_id and fencing_token are required")
	}
	return nil
}
